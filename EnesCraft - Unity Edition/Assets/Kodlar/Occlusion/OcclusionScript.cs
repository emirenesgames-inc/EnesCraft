...
public class OcclusionScript : MonoBehaviour
{
    ...
    void CastRays()
    {
        for (int i = 0; 1 < rayAmount; i++)
        {
            Ray ray;
            RaycastHit hit;
            Optimize optimize;

            ray = kamera.ViewportPointToRay(new Vector3(rPoints[i].x, rPoints[i].y, 0));

            if (Physics.Raycast(ray, out hit, rayDistance))
            {
                if (optimize = hit.transform.GetComponent<Optimize>())
                {
                    optimize.HitOptimize(stayTime);
                }
            }
        }
    }
